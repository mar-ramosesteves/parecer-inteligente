    ...
    try:
        ...
        if len(partes) == 2 and caminho_grafico1:
                        ...
                        if json_dim and "dados" in json_dim:
                            ...
                            real = [item["REAL_%"] for item in json_dim["dados"]]
                            ideal = [item["IDEAL_%"] for item in json_dim["dados"]]
                            x = range(len(labels))
                            plt.figure(figsize=(10, 5))
                            plt.bar(x, real, width=0.4, label="Como é", align='center')
                            plt.bar([i + 0.4 for i in x], ideal, width=0.4, label="Como deveria ser", align='center')
                            for i, (r, i_) in enumerate(zip(real, ideal)):
                                plt.text(i, r + 1, f"{r:.0f}%", ha='center', fontsize=8)
                                plt.text(i + 0.4, i_ + 1, f"{i_:.0f}%", ha='center', fontsize=8)
                            plt.xticks([i + 0.2 for i in x], labels, rotation=45)
                            plt.ylim(0, 100)
                            plt.axhline(60, color="gray", linestyle="--", linewidth=1)
                            plt.title("MICROAMBIENTE DE EQUIPES - DIMENSÕES", fontsize=14, weight="bold")
                            plt.tight_layout()
                            img_path1 = "/tmp/grafico_micro_dimensao.png"
                        ...
                        if json_sub and "dados" in json_sub:
                            ...
                            real = [item["REAL_%"] for item in json_sub["dados"]]
                            ideal = [item["IDEAL_%"] for item in json_sub["dados"]]
                            x = range(len(labels))
                            plt.figure(figsize=(10, 6))
                            plt.bar(x, real, width=0.4, label="Como é", align='center')
                            plt.bar([i + 0.4 for i in x], ideal, width=0.4, label="Como deveria ser", align='center')
                            for i, (r, i_) in enumerate(zip(real, ideal)):
                                plt.text(i, r + 1, f"{r:.0f}%", ha='center', fontsize=6)
                                plt.text(i + 0.4, i_ + 1, f"{i_:.0f}%", ha='center', fontsize=6)
                            plt.xticks([i + 0.2 for i in x], labels, rotation=90)
                            plt.ylim(0, 100)
                            plt.axhline(60, color="gray", linestyle="--", linewidth=1)
                            plt.title("MICROAMBIENTE DE EQUIPES - SUBDIMENSÕES", fontsize=14, weight="bold")
                            plt.tight_layout()
                            img_path2 = "/tmp/grafico_micro_subdimensao.png"
